...
/* VGA modes and fonts by Chris Giese <geezer@execpc.com> http://my.execpc.com/~geezer */
...
static void set_plane(unsigned p)
{
	...
	pmask = 1 << p;
/* set read plane */
	write_port(VGA_GC_INDEX, 4);
	write_port(VGA_GC_DATA, p);
/* set write plane */
	write_port(VGA_SEQ_INDEX, 2);
	write_port(VGA_SEQ_DATA, pmask);
}
...
set_plane() modifies GC 4 and SEQ 2, so save them as well */
	write_port(VGA_SEQ_INDEX, 2);
	seq2 = read_port(VGA_SEQ_DATA);

	write_port(VGA_SEQ_INDEX, 4);
	seq4 = read_port(VGA_SEQ_DATA);
/* turn off even-odd addressing (set flat addressing)
assume: chain-4 addressing already off */
	write_port(VGA_SEQ_DATA, seq4 | 0x04);

	write_port(VGA_GC_INDEX, 4);
	gc4 = read_port(VGA_GC_DATA);

	write_port(VGA_GC_INDEX, 5);
	gc5 = read_port(VGA_GC_DATA);
/* turn off even-odd addressing */
	write_port(VGA_GC_DATA, gc5 & ~0x10);

	write_port(VGA_GC_INDEX, 6);
	gc6 = read_port(VGA_GC_DATA);
/* turn off even-odd addressing */
	write_port(VGA_GC_DATA, gc6 & ~0x02);
...
/* restore registers */
	write_port(VGA_SEQ_INDEX, 2);
	write_port(VGA_SEQ_DATA, seq2);
	write_port(VGA_SEQ_INDEX, 4);
	write_port(VGA_SEQ_DATA, seq4);
	write_port(VGA_GC_INDEX, 4);
	write_port(VGA_GC_DATA, gc4);
	write_port(VGA_GC_INDEX, 5);
	write_port(VGA_GC_DATA, gc5);
	write_port(VGA_GC_INDEX, 6);
	write_port(VGA_GC_DATA, gc6);
}
#endif
