...
static void expand(uint32_t new_size, heap_t *heap) {
	assume(new_size > heap->end_address - heap->start_address);
	if ((new_size&0xFFFFF000) != 0) {
		new_size &= 0xFFFFF000;
		new_size += 0x1000;
	}
	assume(heap->start_address+new_size <= heap->max_address);
	uint32_t old_size = heap->end_address-heap->start_address;
	uint32_t i = old_size;
	while (i < new_size) {
		alloc_page(_get_page(heap->start_address+i));
		i += 0x1000;
	}
...
static uint32_t contract(uint32_t new_size, heap_t *heap) {
	...
	while (new_size < i) {
		free_page(_get_page(heap->start_address+i));
		i -= 0x1000;
	}
	heap->end_address = heap->start_address + new_size;
	return new_size;
...
heap_t *create_heap(uint32_t start, uint32_t end_addr, uint32_t max, uint8_t supervisor, uint8_t readonly) {
	...
	assume(start%0x1000 == 0);
	assume(end_addr%0x1000 == 0);
...
void *_alloc(uint32_t size, uint8_t page_align, heap_t *heap) {
...
void _free(void *p, heap_t *heap) {
	...
	footer_t *footer = (footer_t*) ((uint32_t)header + header->size - sizeof(footer_t));
	assume(header->magic == HEAP_MAGIC);
	assume(footer->magic == HEAP_MAGIC);
	header->is_hole = 1;
	char do_add = 1;
	...
	if (test_header->magic == HEAP_MAGIC && test_header->is_hole) {
		...
		while ((iterator < heap->index.size) && (lookup_ordered_array(iterator, &heap->index) != (void*)test_header))
			iterator++;
		assume(iterator < heap->index.size);
		remove_ordered_array(iterator, &heap->index);
	}
